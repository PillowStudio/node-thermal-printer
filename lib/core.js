...
class ThermalPrinter {
  ...
  execute() {
    return new Promise((resolve, reject) => {
      try {
        this.Interface.execute(this.buffer).then(value => {
          resolve(value);
        });
      } catch (e) {
        reject(e);
  ...
  isPrinterConnected(exists) {
    return new Promise((resolve) => {
      this.Interface.isPrinterConnected(exists).then(value => {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (image.slice(-4) === ".png") {
          try {
            this.printer.printImage(image).then(response => {
          ...
          reject(new Error("Image printing supports only PNG files."));
        }
      } catch (error) {
        reject(error);
      }
  ...
  // ----------------------------------------------------- PRINT IMAGE BUFFER -----------------------------------------------------
  printImageBuffer(buffer) {
    return new Promise((resolve, reject) => {
      try {
        PNG.sync.read(buffer).then(png => {
          var buff = this.printer.printImageBuffer(png.width, png.height, png.data);
          this.append(buff);
          resolve(buff);
        });
      } catch(error) {
  ...
  raw(text) {
    return new Promise((resolve, reject) => {
      try {
        this.Interface.execute(text).then(result => {
          resolve(result)
        });
      } catch (error) {
        reject(error);
